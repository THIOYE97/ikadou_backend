        ...
        screens: [
          {
            ...
            description: 'Parcourez rapidement les opportunités disponibles selon votre budget et votre zone.',
          },
          {
            id: 'trust',
            title: 'Consultez des fiches rassurantes',
            description: 'Photos, prix, surface, disponibilité et informations utiles pour décider sereinement.',
          },
          {
            id: 'action',
            title: 'Passez à l’action au bon moment',
            description: 'Créez votre compte pour contacter, suivre vos demandes et planifier une visite.',
          },
        ],
      },
    });
...
router.get('/terrains/map', async (req, res, next) => {
  try {
...
router.get('/terrains/:id', async (req, res, next) => {
  try {
...
router.get('/terrains/:id/visit-policy', async (req, res, next) => {
  try {
    return res.json({
      success: true,
      data: {
      ...
      if (existing.rows.length) {
        const updated = await query(
          `UPDATE leads
           ...
           WHERE id = $1
           RETURNING *`,
          [existing.rows[0].id, firstName, lastName, email || null, phone, terrainId || null]
        );
        lead = updated.rows[0];
      } else {
        const inserted = await query(
          `INSERT INTO leads
            (first_name, last_name, email, phone, source, status, terrain_id)
           VALUES ($1, $2, $3, $4, 'mobile_app', 'new', $5)
           RETURNING *`,
          [firstName, lastName, email || null, phone, terrainId || null]
        );
  ...
  async (req, res, next) => {
    try {
      const terrainCheck = await query(
        `SELECT id FROM terrains WHERE id = $1 AND status = 'published'`,
        [req.params.id]
      );
      if (!terrainCheck.rows.length) throw HttpError.notFound('Terrain introuvable');

      req.body.terrainId = req.params.id;
      ...
      if (existing.rows.length) {
        const updated = await query(
          `UPDATE leads
           ...
           WHERE id = $1
           RETURNING *`,
          [existing.rows[0].id, req.body.firstName, req.body.lastName, req.body.email || null, req.body.phone, req.params.id]
        );
        lead = updated.rows[0];
      } else {
        const inserted = await query(
          `INSERT INTO leads
           ...
           VALUES ($1, $2, $3, $4, 'mobile_app', 'new', $5)
           RETURNING *`,
          [req.body.firstName, req.body.lastName, req.body.email || null, req.body.phone, req.params.id]
        );
        lead = inserted.rows[0];
      }
